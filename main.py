...
parser.add_argument('-max_ncomm', '--max_num_comm', type=int, default=50, help='maximum number of communication rounds, may terminate early if converges')
...
parser.add_argument('-nm', '--num_malicious', type=int, default=4, help="number of malicious enterprises in the network. Malicious Enterprises data sets will be introduced Gaussian noise")
...
parser.add_argument('-mu', '--mu', type=float, default=0.01, help='FedProx proximal term. Set to 0 to disable.')
...
# detect Accelerator (Refactored for NVIDIA/TPU Prioritization)
if torch.cuda.is_available():
    # Explicitly try to pick the NVIDIA device if multiple exist
    gpu_id = int(args['gpu']) if args['gpu'].isdigit() else 0
    ...
    try:
        dev = xm.xla_device()
        print(f"System: TPU Detected. Using Device: {dev}")
...
if args['resume_path']:
	...
	if dev.type == 'cuda':
		...
		print(f"Current GPU: {torch.cuda.get_device_name(dev)}")
	else:
		print("WARNING: CUDA is not detected. Falling back to CPU. Training will be slow!")
	...
	for comm_round in range(latest_round_num + 1, args['max_num_comm']+1):
		...
		# free cuda/system memory before starting new round
		gc.collect()
		if dev.type == 'cuda':
		...
		for validator_iter in range(len(validators_this_round)):
			...
			# Optimized for 12GB RAM: Memory Cleanup after each validator's intensive validation phase
			del final_transactions_arrival_queue
			gc.collect()
			torch.cuda.empty_cache()
		...
		for miner_iter in range(len(miners_this_round)):
			...
			if HAS_XLA and dev.type == 'xla' and miner_iter % 5 == 0:
				xm.mark_step()
			miner = miners_this_round[miner_iter]
						...
						if verification_time:
							if is_validator_sig_valid:
								...
								if not found_same_local_enterprise_transaction:
									# Optimized for 12GB RAM: Avoid full deepcopy of dictionaries containing model weights
									# We only copy the fields we need
									valid_validator_sig_candidate_transaciton = {k: v for k, v in unconfirmmed_transaction.items() if k not in [
										'validation_done_by', 'validation_rewards', 'update_direction', 
										'validation_time', 'validator_rsa_pub_key', 'validator_signature'
									]}
		...
		for enterprise in enterprises_list:
			accuracy_this_round = enterprise.validate_model_weights(limit_batches=50)
