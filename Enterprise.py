...
# Helper: deepcopy that retains tensors on their original device (GPU) to resolve CPU RAM bloat
def _deepcopy_tensors(obj):
	if isinstance(obj, dict):
		gpu_obj = {}
		for k, v in obj.items():
			if isinstance(v, torch.Tensor):
				gpu_obj[k] = v.detach().clone()
			elif isinstance(v, dict):
				gpu_obj[k] = _deepcopy_tensors(v)
			else:
				gpu_obj[k] = copy.deepcopy(v)
		return gpu_obj
...
class Enterprise:
	def __init__(self, idx, assigned_train_ds, assigned_test_dl, local_batch_size, learning_rate, loss_func, opti, network_stability, net, dev, miner_acception_wait_time, miner_accepted_transactions_size_limit, validator_threshold, pow_difficulty, even_link_speed_strength, base_data_transmission_speed, even_computation_power, is_malicious, noise_variance, check_signature, not_resync_chain, malicious_updates_discount, knock_out_rounds, lazy_local_enterprise_knock_out_rounds):
		...
		if opti == "SGD":
			...
			self.opti = optim.Adam(self.net.parameters(), lr=learning_rate, betas=(0.9, 0.9), weight_decay=1e-4)
		self.dev = dev
		# Mixed Precision Training scaler for GPU speedup
		...
		if HAS_XLA and self.dev.type == 'xla':
			try:
				# Move all underlying tensors in the dataset to TPU memory
				self.train_dl.dataset.tensors = [t.to(self.dev) for t in self.train_dl.dataset.tensors]
				if self.test_dl and hasattr(self.test_dl.dataset, 'tensors'):
					self.test_dl.dataset.tensors = [t.to(self.dev) for t in self.test_dl.dataset.tensors]
				print(f"Enterprise {self.idx} pre-loaded datasets to {self.dev} (TPU/XLA Optimization)")
			except Exception as e:
				print(f"Warning: Failed to pre-load dataset to TPU for {self.idx}: {e}")
	...
	def verify_miner_transaction_by_signature(self, transaction_to_verify, miner_enterprise_idx):
		...
		if self.check_signature:
			# Optimized: Avoid deepcopy of heavy model parameters
			transaction_before_signed = {k: v for k, v in transaction_to_verify.items() if k != "miner_signature"}
	...
	def verify_block(self, block_to_verify, sending_miner):
		...
		if self.check_signature:
			...
			# verify signature
			# Optimized: Use sorted items for deterministic hash and avoid deepcopy
			block_content = block_to_verify.return_block_dict_for_signature()
			hash = int.from_bytes(sha256(str(sorted(block_content.items())).encode('utf-8')).digest(), byteorder='big')
			hashFromSignature = pow(signature, pub_key, modulus)
			if hash != hashFromSignature:
		...
		for mt in self.model_type:
			...
			for epoch in range(local_epochs):
				for data, label in self.train_dl:
					...
					self.scaler.update()
					self.opti.zero_grad(set_to_none=True)
					self.local_updates_rewards_per_transaction += rewards * (label.shape[0])
				# TPU Sync at the end of epoch instead of per-batch for 10x speedup
				if HAS_XLA and self.dev.type == 'xla':
					xm.mark_step()
	...
	def return_local_updates_and_signature(self, comm_round):
		...
		local_updates_dict = {'local_enterprise_enterprise_idx': self.idx, 'in_round_number': comm_round, "local_updates_params": _deepcopy_tensors(self.local_train_parameters), "local_updates_rewards": self.local_updates_rewards_per_transaction, "local_iteration(s)_spent_time": self.local_update_time, "local_total_accumulated_epochs_this_round": self.local_total_epoch, "local_enterprise_rsa_pub_key": self.return_rsa_pub_key()}
	...
	def validate_model_weights(self, weights_to_eval=None, limit_batches=None):
		"""
		Turbo Mode: Optimized validation with optional batch limiting.
		"""
		with torch.no_grad():
			if weights_to_eval:
			...
			sum_accu = 0
			num = 0
			for i, (data, label) in enumerate(self.test_dl):
	...
	def global_update(self, local_update_params_potentially_to_be_used): #global update
		...
		if local_params_by_benign_local_enterprises:
			# Optimized: Removed redundant nested loop that was manually calculating similarity on CPU.
			# Aggregation is now handled by vectorized GPU/TPU-native logic to avoid massive data transfer overhead.
			pass
			# FedAnil+: Clustered Aggregation via Models helper
			try:
				...
				if num_clusters > 0:
					...
					print(f"Global updates (Clustered) done by {self.idx}")
					self.global_time = time.time() - self.global_time
					return
			except Exception as e:
				print(f"Clustered aggregation failed: {e}. Falling back to FedAvg.")
				
			# Fallback: Filtered FedAvg (Optimized for 12GB RAM)
			num_participants = 0
			sum_parameters = {}
			for idx, lp in local_update_params_potentially_to_be_used:
				if idx in self.black_list:
					continue
			...
			if num_participants > 0:
				...
				print(f"Global updates (Fallback FedAvg) done by {self.idx}")
		...
		# Explicitly cleanup local model copies to free System RAM
		try:
			del local_params_by_benign_local_enterprises
		except:
			pass
		gc.collect()
		self.global_time = time.time() - self.global_time
	...
	def request_to_download(self, block_to_download, requesting_time_point):
		...
		for enterprise in enterprises_in_association:
			...
			if self.online_switcher() and enterprise.online_switcher():
				...
				# Optimized: Estimate size without full string conversion to save RAM
				transmission_delay = getsizeof(block_to_download)/lower_link_speed
	...
	def verify_validator_transaction(self, transaction_to_verify):
		if self.computation_power == 0:
			...
			if self.check_signature:
				# Optimized: Avoid deepcopy of heavy model parameters
				transaction_before_signed = {k: v for k, v in transaction_to_verify.items() if k != "validator_signature"}
...
#		self.block_to_add = None
	...
	def accept_miner_broadcasted_validator_transactions(self, source_enterprise, unordered_transaction_arrival_queue_from_source_miner):
		...
		if not source_enterprise.return_idx() in self.black_list:
			self.miner_accepted_broadcasted_validator_transactions.append({'source_enterprise_link_speed': source_enterprise.return_link_speed(),'broadcasted_transactions': _deepcopy_tensors(unordered_transaction_arrival_queue_from_source_miner)})
	...
	def sign_block(self, block_to_sign):
		# Optimized: Use sorted items for deterministic hash matching verification logic
		block_content = block_to_sign.return_block_dict_for_signature()
		block_to_sign.set_signature(self.sign_msg(sorted(block_content.items())))
	...
	def add_unconfirmmed_transaction(self, unconfirmmed_transaction, souce_enterprise_idx):
		if not souce_enterprise_idx in self.black_list:
			self.unconfirmmed_transactions.append(_deepcopy_tensors(unconfirmmed_transaction))
	...
	def accept_broadcasted_transactions(self, source_enterprise, broadcasted_transactions):
		# discard malicious node
		if not source_enterprise.return_idx() in self.black_list:
			self.broadcasted_transactions.append(_deepcopy_tensors(broadcasted_transactions))
	...
	def accept_validator_broadcasted_local_enterprise_transactions(self, source_validator, unordered_transaction_arrival_queue_from_source_validator):
		if not source_validator.return_idx() in self.black_list:
			self.validator_accepted_broadcasted_local_enterprise_transactions.append({'source_validator_link_speed': source_validator.return_link_speed(),'broadcasted_transactions': _deepcopy_tensors(unordered_transaction_arrival_queue_from_source_validator)})
	...
	def validate_local_enterprise_transaction(self, transaction_to_validate, rewards, log_files_folder_path, comm_round, malicious_validator_on):
		...
		if self.computation_power == 0:
			...
			if self.check_signature:
				# Optimized: Avoid deepcopy of heavy model parameters
				transaction_before_signed = {k: v for k, v in transaction_to_validate.items() if k != "local_enterprise_signature"}
			...
			if transaction_to_validate['local_enterprise_signature_valid']:
				# Accuracy Fix: 50 batches (~6400 images) = reliable accuracy + memory safe
				accuracy_by_validator_updated_model = self.validate_model_weights(limit_batches=50)
				print(f"validator updated model accuracy - {accuracy_by_validator_updated_model}")
				
				# Ensure parameters are on the right device
				local_updates_params = {k: v.to(self.dev) for k, v in transaction_to_validate["local_updates_params"].items()}
				
				accuracy_by_applying_local_enterprise_update = self.validate_model_weights(local_updates_params, limit_batches=50)
				
				print(f'validator updated model accuracy - {accuracy_by_validator_updated_model}')
				print(f"After applying local_enterprise's update, model accuracy becomes - {accuracy_by_applying_local_enterprise_update}")
				...
				if accuracy_by_applying_local_enterprise_update - accuracy_by_validator_updated_model < self.validator_threshold * -1:
					...
					if not self.enterprises_dict[local_enterprise_transaction_enterprise_idx].return_is_malicious():
						...
						with open(f"{log_files_folder_path}/false_negative_good_nodes_inside_victims.txt", 'a') as file:
							file.write(f"{accuracy_by_validator_updated_model - accuracy_by_applying_local_enterprise_update} = current_validator_accuracy {accuracy_by_validator_updated_model} - accuracy_by_applying_local_enterprise_update {accuracy_by_applying_local_enterprise_update} , by validator {self.idx} on local_enterprise {local_enterprise_transaction_enterprise_idx} in round {comm_round}\n")
					else:
					...
					if self.enterprises_dict[local_enterprise_transaction_enterprise_idx].return_is_malicious():
						...
						# for experiments
						with open(f"{log_files_folder_path}/false_positive_malerious_nodes_inside_slipped.txt", 'a') as file:
							file.write(f"{accuracy_by_validator_updated_model - accuracy_by_applying_local_enterprise_update} = current_validator_accuracy {accuracy_by_validator_updated_model} - accuracy_by_applying_local_enterprise_update {accuracy_by_applying_local_enterprise_update} , by validator {self.idx} on local_enterprise {local_enterprise_transaction_enterprise_idx} in round {comm_round}\n")
					else:
